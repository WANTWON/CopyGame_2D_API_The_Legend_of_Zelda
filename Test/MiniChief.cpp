...
CMiniChief::CMiniChief() : m_bCount(false)
{
...
CMiniChief::~CMiniChief()
{
	Release();
}

void CMiniChief::Initialize(void)
{
	CTileMgr::Get_Instance()->Load_Tile(SC_MINICHIEF);
	CBlockMgr::Get_Instance()->Load_Block(SC_MINICHIEF);

	CObj* pObj9 = CAbstractFactory<CNpc>::Create(440, 300);
	static_cast<CNpc*>(pObj9)->Set_NPCType(MINI_CHIEF);
	...
	else
	{
		...
		CObjMgr::Get_Instance()->Get_Player()->Set_Pos(400, 370);

	}
}

int CMiniChief::Update(void)
{
...
void CMiniChief::Late_Update(void)
{
	...
	if (!m_bCount)
	{
		CObj* pObj = CAbstractFactory<CPortal>::Create(400, 516);
...
void CMiniChief::Render(HDC hDC)
{
	int iScrollX = (int)CScrollMgr::Get_Instance()->Get_ScrollX();
	int iScrollY = (int)CScrollMgr::Get_Instance()->Get_ScrollY();


	HDC hGroundDC = CBmpMgr::Get_Instance()->Find_Image(L"MiniChief");
