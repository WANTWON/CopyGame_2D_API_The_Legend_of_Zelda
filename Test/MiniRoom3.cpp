...
CMiniRoom3::CMiniRoom3() : m_bCount(false)
{
...
CMiniRoom3::~CMiniRoom3()
{
	Release();
}

void CMiniRoom3::Initialize(void)
{
	CTileMgr::Get_Instance()->Load_Tile(SC_MINIROOM3);
	CBlockMgr::Get_Instance()->Load_Block(SC_MINIROOM3);

	CObj* pObj9 = CAbstractFactory<CNpc>::Create(440, 300);
	static_cast<CNpc*>(pObj9)->Set_NPCType(MINI_NPC3);
	...
	else
	{
		...
		CObjMgr::Get_Instance()->Get_Player()->Set_Pos(400, 340);
...
void CMiniRoom3::Late_Update(void)
{
	...
	if (!m_bCount)
	{
		CObj* pObj = CAbstractFactory<CPortal>::Create(400, 460);
...
void CMiniRoom3::Render(HDC hDC)
{
	int iScrollX = (int)CScrollMgr::Get_Instance()->Get_ScrollX();
	int iScrollY = (int)CScrollMgr::Get_Instance()->Get_ScrollY();


	HDC hGroundDC = CBmpMgr::Get_Instance()->Find_Image(L"MiniRoom3");
